...
function pageNumberCreator(pageBlock){
    ...
    if(filter.filteredHotels.length>0 && !document.querySelector(".pagination.show")){
        pageinationEl.className += " show";
    }
    if(filter.filteredHotels.length==0){
        pageinationEl.className = "pagination";
    }
    totalPages = Math.ceil(filter.filteredHotels.length/quantity);
